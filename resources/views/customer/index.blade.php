...
@endphp
@section('card-tools')
<button type="button" class="btn btn-success" onclick="loadViewInHome('{{route('customer.create')}}')"><i class="fas fa-plus"></i> Adicionar Cliente</button>
@endsection
@section('card-body')
@if (count($data) > 0)
<table id="example1" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$item->phone}}</td>
            <td>
                <button class="btn btn-xs btn-info" onclick="loadViewInHome('{{route('customer.edit',$item->id)}}')"><i class="fa fa-edit"></i></button>
...
<script>
    ...
    $('#exampleModal').on('show.bs.modal', function(event) {
        ...
        $("form#delete").attr('data-sendrequest', '/customer/' + Id)
    });
</script>
@endsection
